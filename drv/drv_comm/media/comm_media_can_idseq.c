...
/* 总线级自恢复探测的最小间隔（ms）：本后端的**发送入口**每次都会先看一眼（见 MediaCanIdseqProbeBus），
...
#ifndef DRV_COMM_MEDIA_CAN_IDSEQ_LOG_LIMIT
#define DRV_COMM_MEDIA_CAN_IDSEQ_LOG_LIMIT 10
#endif // !DRV_COMM_MEDIA_CAN_IDSEQ_LOG_LIMIT
LOG_INSTANCE_DEF(g_media_can_idseq_log, "comm_media_can_idseq", DRV_COMM_MEDIA_CAN_IDSEQ_LOG_LIMIT);

/* 每条总线（can_e）上一次探测的时刻。
 * @note 限频按**外设**去重而不是按实例：CANRecover 作用于整个外设（它取消的是该总线上
 *       所有实例的在途帧），同一条总线上挂多个东西时（如 gimbal 的 CAN_1 上既有云台↔底盘
 *       链路、又有电机），一个共享节拍才是对的——各实例各自为政只会让"谁的探测把谁的帧
 *       取消了"变得不可预测。 */
...
static void MediaCanIdseqTxHook(CANInstance *can, uint32_t tx_mailbox, BSP_Status_e result);
static void MediaCanIdseqErrHook(CANInstance *can, CAN_ErrReason_e reason);
static void MediaCanIdseqRxHook(CANInstance *can, const CAN_Pack_s *pack);
static void MediaCanIdseqProbeBus(CANInstance *can);
...
static int8_t MediaCanIdseqSend(CommMedia *media, const uint8_t *data)
{
     ...
     * 若把探测放在它后面就永远够不到了。判据/动作/限频的理由见 MediaCanIdseqProbeBus。 */
    MediaCanIdseqProbeBus(can);

    if (m->tx_active)
    {
        /* 卡死兜底：分包续发依赖 bsp 发送完成回调，一旦该回调丢失（如邮箱 RQCP 被
         * 同总线其他实例复用吞掉 / 总线异常），tx_active 会永久为 1，之后所有 Send
         * 都被拒 → 链路单向静默。连续 N 次 Send 仍见 tx_active=1 即判定卡死，强制
         * 放弃残帧（接收端按 seq 跳号丢帧重同步，仅损失一帧）后继续发新帧。 */
        if (++m->tx_stall >= CAN_MEDIA_IDSEQ_TX_STALL_LIMIT)
        {
            m->tx_stall = 0;
            m->tx_active = 0;
            m->tx_sent = 0;
    ...
    m->tx_sent = 0;
    m->tx_active = 1;
    m->tx_stall = 0; /* 新帧开始，卡死计数归零（单帧正常耗时不会累积到阈值） */
...
static void MediaCanIdseqTxHook(CANInstance *can, uint32_t tx_mailbox, BSP_Status_e result)
{
     ...
     * 旧实现只有"发送成功"才回调，失败时什么也不知道，只能等 CAN_MEDIA_IDSEQ_TX_STALL_LIMIT
    ...
    r = MediaCanIdseqSendNext(m);
    if (r != 0)
    {
        m->tx_active = 0; /* 发完(1) 或失败(-1)：结束本轮异步发送 */
        if (r < 0)
            m->tx_fail++; /* 分包续发失败：记录（接收端按 seq 跳号丢帧重同步） */
    }
}
 ...
 *       跨实例误伤。真丢了完成回调的情形由 CAN_MEDIA_IDSEQ_TX_STALL_LIMIT 兜底。
 *       err_callback 是广播（同一条总线上的所有实例都会收到），所以 handler 里
 *       不能有任何"只对自己成立"的假设。 */
static void MediaCanIdseqErrHook(CANInstance *can, CAN_ErrReason_e reason)
{
    CommMediaCanIdseq *m = (CommMediaCanIdseq *)can->parent;
 ...
 * @note 本仓库最需要它的一条总线是 gimbal 的 CAN_1（底盘链路 + 电机共用）：判据成立时
 *       取消的是整条总线上的在途帧，故限频按 can_e 共享（见 s_can_recover_us）。
...
int8_t MediaCanIdseqConfig(CommMediaCanIdseq *media, CommMediaCanIdseqConfig_s *cfg)
{
    ...
    can_cfg = (CAN_Config_s){
        ...
        .filter_num = 1,
        .tx_complete_callback = MediaCanIdseqTxHook, /* 发送完成回调：逐包续发 + 逐帧失败当场收尾 */
        .err_callback = MediaCanIdseqErrHook,        /* 总线/硬件级错误：仅观测计数（见 ErrHook 说明） */
    };
