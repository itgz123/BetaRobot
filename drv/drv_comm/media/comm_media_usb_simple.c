...
#ifndef DRV_COMM_MEDIA_USB_SIMPLE_LOG_LIMIT
#define DRV_COMM_MEDIA_USB_SIMPLE_LOG_LIMIT 10
#endif // !DRV_COMM_MEDIA_USB_SIMPLE_LOG_LIMIT
LOG_INSTANCE_DEF(g_media_usb_simple_log, "comm_media_usb_simple", DRV_COMM_MEDIA_USB_SIMPLE_LOG_LIMIT);

static int8_t MediaUsbSimpleSend(CommMedia *media, const uint8_t *data);
static void MediaUsbSimpleRxHook(USBInstance *usb);
static void MediaUsbSimpleErrHook(USBInstance *usb, USB_ErrReason_e reason);
static void MediaUsbSimpleOfflineHook(void *owner);
...
static const CommMediaVTable_s s_usb_simple_vtable = {
    ...
    .offline = MediaUsbSimpleOfflineHook,
};
...
static int8_t MediaUsbSimpleSend(CommMedia *media, const uint8_t *data)
{
    ...
    {
        st = USBTransmit(usb, data, m->tx_frame_len);
        if (st == BSP_BUSY)
    ...
    while (remain > 0)
    {
        ...
        if (st != BSP_OK)
        {
            /* BSP_BUSY = ring 放不下这一包（bsp 保证零字节写入）→ 背压；
             * 其余 = 本帧确实发不出去。两者分开计数，见 comm_media_usb.c 的说明。 */
            if (st == BSP_BUSY)
                m->tx_busy++;
            else
                m->tx_fail++;
...
static void MediaUsbSimpleOfflineHook(void *owner)
{
    ...
    CommMediaUsbSimple *m;
    USBInstance *usb;

    if (inst == NULL || inst->media == NULL)
        return;

    m = (CommMediaUsbSimple *)inst->media; /* vtable 只挂在 USB-simple 介质上，可达 */
    usb = (USBInstance *)m->base.media;
    if (usb == NULL)
        return; /* 介质未绑定 bsp 实例 */
    ...
    if (USBRecoverTxIfStuck(usb, 0) == BSP_OK)
        BSPLOG(&g_media_usb_simple_log, LOG_LEVEL_WARNING, "TX stuck, flushed by offline hook");

    /* 接收侧：久无收帧则重新武装 OUT 端点 */
    if (USBRecoverRxIfStalled(usb, DRV_COMM_MEDIA_USB_RX_RESTART_PERIOD_MS) == BSP_OK)
        BSPLOG(&g_media_usb_simple_log, LOG_LEVEL_WARNING, "RX stalled, re-armed by offline hook");
}
...
int8_t MediaUsbSimpleConfig(CommMediaUsbSimple *media, USB_Config_s *cfg)
{
    ...
    local_cfg.err_callback = MediaUsbSimpleErrHook;    /* 本层观测计数 + 转发（见 ErrHook 说明） */
