 ...
 * @note 上面的"8bit"只是列出来对照手册，**代码里一律只用 7 位的这个宏**：
 *       送给 bsp_i2c 的是设备地址本身，左移成 HAL 需要的 8 位形式是 bsp 的活
 *       （见 bsp_i2c.h「从机地址形态」）。旧版本此处曾有个 `I2C_DEV_ADDR()` 别名宏，
 *       已随该约定删除。
 */
#define IST8310_I2C_ADDR_7BIT 0x0E // 从机地址（7 位形式，直接传给 bsp_i2c）
