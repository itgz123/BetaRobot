 ...
 * @param tx_fail    出参：失败时自增的计数器；可传 NULL（见下）
 ...
 *       上层按低频率读，这才是能用的观测通道。
 *
 * @note **组播帧传 tx_fail = NULL**：DJI / LK 的广播帧一帧带 4 个电机（`0x1FF`/`0x200`/`0x280`），
 *       它不属于任何单个电机实例。这类帧的发送失败归属由 bsp 层的 per-CAN 计数器承担
 *       （`s_bxcan_status[].tx_fail` / `s_fdcan_status[].tx_fail`）。
 */
