...
int8_t BMI088SetConfig(BMI088Instance *inst, BMI088_AccRange_e acc_range, uint8_t acc_bwp, uint8_t acc_odr, BMI088_GyroRange_e gyro_range, uint8_t gyro_odr, uint8_t gyro_bw, BMI088_WorkMode_e work_mode)
{
    ...
    // 阻塞模式
    if (work_mode == BMI088_MODE_POLLING)
    {
        /*============================ 加速度计初始化 ============================*/
        ...
        // 4. 检查 CHIP_ID
        BMI088_AccReadReg(inst, BMI088_ACC_CHIP_ID_REG, 1);
        LOGINFO("BMI088 Acc CHIP_ID: 0x%02X", inst->spi_inst.rx_buff[2]);
        if (inst->spi_inst.rx_buff[2] != BMI088_ACC_CHIP_ID_VAL)
        {
            LOGERROR("BMI088 Acc CHIP_ID error: 0x%02X", inst->spi_inst.rx_buff[2]);
            return -1;
        }
        DWT_Delay(BMI088_DELAY_MS);
        ...
        {
            LOGERROR("BMI088 acc_pwr_ctrl write failed");
            return -1;
        }
        ...
        {
            LOGERROR("BMI088 acc_pwr_conf write failed");
            return -1;
        }
        ...
        LOGINFO("BMI088 Gyro CHIP_ID: 0x%02X", inst->spi_inst.rx_buff[1]);
        if (inst->spi_inst.rx_buff[1] != BMI088_GYRO_CHIP_ID_VALUE)
        {
            LOGERROR("BMI088 Gyro CHIP_ID error: 0x%02X", inst->spi_inst.rx_buff[1]);
            return -1;
        }
    ...
    {
        LOGWARNING("It's not support now!");
        // 初始化完毕，切换为dma模式
