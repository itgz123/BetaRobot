...
void DWT_Init(void)
{
    ...
    /* 预计算 us 换算因子（魔数法，见 lib_math_int64.h）：
     ...
     * 定点小数字因子：us = cyccnt64 × (magic/2^64) = Lib_Math_U64Div(...)，
     * 全 umull+移位+减法，零除法指令。 */
    s_cycles_per_us = SystemCoreClock / 1000000u;
    s_us_magic = Lib_Math_Magic(s_cycles_per_us);
...
uint64_t DWT_GetTimeUs(void)
{
    ...
    return Lib_Math_U64Div(cyccnt64, s_cycles_per_us, s_us_magic);
}
