...
### 3.4.1 为什么本入口不需要"入口自证"（与 I2C / CAN 的分界）

同样是"总线类型的 recover 入口"，`I2CBusRecover` 与 `CANRecover` 都要在入口**先自证总线级
判据、不成立就早退 `BSP_BUSY`**（见 `bsp_i2c.md` §2.B.1.1、`bsp_can.md` §6.4.1），本入口不用。
分界不在"是哪个外设"，而在**判据的作用域与动作的作用域是否重合**：

|         | 判据读什么                              | 动作作用于谁                            | 是否重合 |
| ------- | --------------------------------------- | --------------------------------------- | -------- |
| SPI     | 本句柄 `State` + 本句柄两路 DMA 流（外设自己说的话） | 本句柄（`HAL_SPI_Abort` + 强制放流），中止的正是判据认定的那笔卡死传输 | **是** —— 无需自证 |
| I2C     | 触发者只能给出"这个从机没应答"（**实例级**转述） | 整个外设重建（DeInit/Init：时钟、GPIO、NVIC） | 否 —— 判据必须换成 `I2C_FLAG_BUSY` 且句柄空闲 |
| CAN     | 触发者只能给出"我这条链路没收到帧"（**实例级**转述） | 取消该总线上**所有实例**的在途帧、必要时停/启外设 | 否 —— 判据必须换成 bus-off / 发送资源满且 TEC≥128 |

不重合时，把判据交给调用点就等于让"实例级现象"去驱动"总线级破坏"：对端不发、滤波器不匹配、
只是赶上一次噪声，都会成立，而恢复一个都治不好，代价却是打断同总线别的实例的在途传输。

**代价的边界**：本入口中止的那一笔是不是"正是卡死的那笔"？是 —— 判据是"`State`（含 DMA 流）
连续非 READY 超过 20ms"，而一次正常传输只有 6~64µs（§3.4 阈值取值理由），量级差三个数量级，
在途的正常传输不会被误判。这也是 `stuck_ms` 由调用方传、却只算"收紧判据"的原因：
`drv_bmi088` 传 1ms 是这条链路愿意更早认输，判据本身与作用范围仍由本层定。
...
- ~~后续：can / usb 最后迁到 `BSP_Status_e`~~ **已完成**（iic 早已按本模板迁完，见 `bsp_i2c.md`；
  ...
  三个模块的共同点：失败返回码分开、错误回调只在动作之后、恢复入口由调用方按自己的
  计数/时基触发（SPI 传判定阈值 `stuck_ms`，I2C 由 DRV 侧 500ms 冷却，CAN 由 media 的
  发送入口 100ms 限频），**判据与动作都在 bsp，上层只管"何时看一眼"**。
