...
int8_t CANConfig(CANInstance *instance, const CAN_Config_s *config)
{
    ...
    BSP_RETURN_IF_TRUE_LOG(hfdcan == NULL, -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "FDCAN handle is NULL, check bsp_map mapping!"));
    ...
    if (!s_fdcan_started[instance->can_e])
    {
        ...
        BSP_RETURN_IF_TRUE_LOG(init->TxFifoQueueElmtsNbr == 0, -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "TxFifoQueueElmtsNbr=0! 统一接口发送走 Tx FIFO，请用 CubeMX 配置（TxFifoQueueElmtsNbr>0）"));
        BSP_RETURN_IF_TRUE_LOG(init->TxFifoQueueMode != FDCAN_TX_FIFO_OPERATION, -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "TxFifoQueueMode 需为 FDCAN_TX_FIFO_OPERATION（只要 Tx FIFO，不要 Tx Queue）!"));

        // 硬过滤全通：全部走 FDCAN_ACCEPT_IN_RX_FIFO0（接收只保留 Rx FIFO 路径）
        BSP_RETURN_IF_TRUE_LOG(init->RxFifo0ElmtsNbr == 0, -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "RxFifo0ElmtsNbr=0! 接收走 Rx FIFO0，请用 CubeMX 配置（RxFifo0ElmtsNbr>0）"));
        BSP_RETURN_IF_TRUE_LOG(HAL_FDCAN_ConfigGlobalFilter(hfdcan, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_FILTER_REMOTE, FDCAN_FILTER_REMOTE) != HAL_OK,
                               -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "HAL_FDCAN_ConfigGlobalFilter failed!"));
        ...
        BSP_RETURN_IF_TRUE_LOG(HAL_FDCAN_ConfigInterruptLines(hfdcan, line0_ints, FDCAN_INTERRUPT_LINE0) != HAL_OK,
                               -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "HAL_FDCAN_ConfigInterruptLines failed!"));
        ...
        BSP_RETURN_IF_TRUE_LOG(HAL_FDCAN_Start(hfdcan) != HAL_OK, -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "HAL_FDCAN_Start failed!"));
        BSP_RETURN_IF_TRUE_LOG(HAL_FDCAN_ActivateNotification(hfdcan, active_it, 0) != HAL_OK, -1, BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "HAL_FDCAN_ActivateNotification failed!"));
...
int8_t CANTransmit(CANInstance *instance, const CAN_Pack_s *pack, uint32_t timeout_ms, uint32_t *tx_mailbox, uint8_t *tx_free_level)
{
    ...
    BSP_RETURN_IF_TRUE_LOG(instance == NULL, CAN_FdcanTxFailThenRet(instance), BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "Instance is NULL!"));
    hfdcan = instance->map.handle;
    BSP_RETURN_IF_TRUE_LOG(hfdcan == NULL, CAN_FdcanTxFailThenRet(instance), BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "FDCAN handle is NULL!"));
    BSP_RETURN_IF_TRUE_LOG(pack == NULL, CAN_FdcanTxFailThenRet(instance), BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "Pack is NULL!"));

    // 长度校验：经典 CAN 单帧最大 8 字节，FD 最大 64 字节
    BSP_RETURN_IF_TRUE_LOG(pack->len > 64, CAN_FdcanTxFailThenRet(instance), BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "Length %d exceeds FD max (64)!", pack->len));
    ...
    if (HAL_FDCAN_GetTxFifoFreeLevel(hfdcan) == 0)
    {
        ...
        else
        {
            ...
            while (HAL_FDCAN_GetTxFifoFreeLevel(hfdcan) == 0)
            {
                ...
                {
                    BSPLOG(&g_can_log, LOG_LEVEL_WARNING, "FDCAN Tx FIFO timeout (can_e=%d, id=0x%lX)!", instance->can_e, (unsigned long)pack->id);
                    return CAN_FdcanTxFailThenRet(instance);
                }
            }
        }
...
void HAL_FDCAN_TxEventFifoCallback(FDCAN_HandleTypeDef *hfdcan, uint32_t TxEventFifoITs)
{
    ...
    if (TxEventFifoITs & (FDCAN_IT_TX_EVT_FIFO_FULL | FDCAN_IT_TX_EVT_FIFO_ELT_LOST))
    {
        ...
        BSPLOG(&g_can_log, LOG_LEVEL_ERROR, "Tx Event FIFO full/lost! 发送完成溯源可能丢失");
    }
