 ...
 *
 * 支持的转换说明（5 种，整数均可带可选长度修饰符 l/ll 表示 64 位）：
 ...
 *     可正常显示（值域不超 32 位）。
 *   - 带 l/ll 修饰符按 64 位读取：%ld 读 int64_t，%lu/%llu 读 uint64_t，
 *     %lx/%llx 读 uint64_t（十六进制）。本模块 l/ll 统一按 long long 语义
 *     处理 64 位，与 ARM ILP32 上 long=32 位的 libc 约定不同（仅本模块内部一致）。
 *   - 其余长度修饰符（h/z/...）不识别，按字面输出。
 ...
 *
 * 明确不支持：%c %i %% %f %p 及 flags(-/0/+/空格/#)、长度修饰符(h/z/...)、
 *   精度(.N)、动态宽度(*)。未支持的转换说明按字面输出该字符（容错，不崩）；
 *   注意格式串中若出现 %%，两个 % 都会按字面输出。l/ll 长度修饰符已支持（见上）。
 *
