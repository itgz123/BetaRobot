...
# 选档走头文件高层入口：-DBSP_MATH_TRIG_LUT_STANDALONE 跳过 app_cfg.h（PC 端不引入
...
# SPEED/PREC → 表区间数 SIZE（与 bsp_math_trig_lut.h 高层映射一致）
size_for() {
    case "$1:$2" in
...
test_one() {
    ...
    gcc -O2 -Wall -Wextra -std=c11 \
        -DBSP_MATH_TRIG_LUT_USED \
        -DBSP_MATH_TRIG_LUT_STANDALONE \
        -DBSP_MATH_TRIG_LUT_SPEED="$sp" \
        -DBSP_MATH_TRIG_LUT_PREC="$pr" \
