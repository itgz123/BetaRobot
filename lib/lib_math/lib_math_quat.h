...
#ifndef __LIB_MATH_QUAT_H
#define __LIB_MATH_QUAT_H
 ...
 */
static inline quaternion_t Lib_Math_QuatIdentity(void)
{
 ...
 */
static inline quaternion_t Lib_Math_QuatMul(quaternion_t q1, quaternion_t q2)
{
    quaternion_t result;
 ...
 * @note 单位四元数的共轭等于其逆
 */
static inline quaternion_t Lib_Math_QuatConjugate(quaternion_t q)
{
    quaternion_t result;
    result.w = q.w;
    result.x = -q.x;
 ...
 * @brief 四元数模长 |q|
 */
static inline float Lib_Math_QuatLength(quaternion_t q)
{
    return Lib_Math_Sqrt(q.w * q.w + q.x * q.x + q.y * q.y + q.z * q.z);
}
 ...
 * @note 零四元数返回单位四元数
 */
static inline quaternion_t Lib_Math_QuatNormalize(quaternion_t q)
{
    ...
    float inv_len = 1.0f / Lib_Math_Sqrt(len_sq);
    quaternion_t result;
 ...
 * @note 对于单位四元数，逆 = 共轭
 */
static inline quaternion_t Lib_Math_QuatInverse(quaternion_t q)
{
    float len_sq = q.w * q.w + q.x * q.x + q.y * q.y + q.z * q.z;
    if (len_sq < 1e-8f)
    {
 ...
 */
static inline vector3_t Lib_Math_QuatRotateVector(quaternion_t q, vector3_t v)
{
    vector3_t q_vec = {q.x, q.y, q.z};

    /* t = 2 * cross(q_vec, v) */
    vector3_t t = Lib_Math_Vec3Cross(q_vec, v);
    t = Lib_Math_Vec3Scale(t, 2.0f);
 ...
 */
static inline euler_t Lib_Math_QuatToEuler(quaternion_t q)
{
    euler_t e = {0};
    ...
    e.roll = Lib_Math_Atan2(sinr_cosp, cosr_cosp);
    ...
    e.yaw = Lib_Math_Atan2(siny_cosp, cosy_cosp);
 ...
 */
static inline quaternion_t Lib_Math_EulerToQuat(euler_t e)
{
 ...
 */
static inline quaternion_t Lib_Math_QuatSlerp(quaternion_t q1, quaternion_t q2, float t)
{
    ...
    if (cos_omega > 0.9999f)
    {
        ...
        return Lib_Math_QuatNormalize(result);
    }
