...
#ifndef __LIB_MATH_MATRIX_H
#define __LIB_MATH_MATRIX_H

#include "lib_math_types.h"
#include "lib_math_trig.h"
#include "lib_math_const.h"
 ...
 * @brief 3x3 单位矩阵
 */
static inline matrix3_t Lib_Math_Mat3Identity(void)
{
    matrix3_t m = {0};
    m.data[0][0] = 1.0f;
    m.data[1][1] = 1.0f;
 ...
 */
static inline matrix3_t Lib_Math_Mat3Mul(matrix3_t m1, matrix3_t m2)
{
    matrix3_t result = {0};
    int i, j, k;
    for (i = 0; i < 3; i++)
 ...
 * @brief 3x3 矩阵转置
 */
static inline matrix3_t Lib_Math_Mat3Transpose(matrix3_t m)
{
    matrix3_t result;
    int i, j;
    for (i = 0; i < 3; i++)
 ...
 * @return 行列式值
 */
static inline float Lib_Math_Mat3Determinant(matrix3_t m)
{
 ...
 */
static inline matrix3_t Lib_Math_Mat3Inverse(matrix3_t m)
{
    float det = Lib_Math_Mat3Determinant(m);
    if (Lib_Math_Fabs(det) < 1e-12f)
    {
        return Lib_Math_Mat3Identity();
    }
 ...
 */
static inline vector3_t Lib_Math_Mat3MulVec3(matrix3_t m, vector3_t v)
{
    vector3_t result;
 ...
 */
static inline matrix3_t Lib_Math_Mat3RotationX(float angle)
{
    float c = Lib_Math_Cos(angle);
    float s = Lib_Math_Sin(angle);
    matrix3_t m = {0};
    m.data[0][0] = 1.0f;
    m.data[1][1] = c;
    m.data[1][2] = -s;
 ...
 * @param angle 旋转角度 (rad)
 */
static inline matrix3_t Lib_Math_Mat3RotationY(float angle)
{
    float c = Lib_Math_Cos(angle);
    float s = Lib_Math_Sin(angle);
    matrix3_t m = {0};
 ...
 * @param angle 旋转角度 (rad)
 */
static inline matrix3_t Lib_Math_Mat3RotationZ(float angle)
{
    float c = Lib_Math_Cos(angle);
    float s = Lib_Math_Sin(angle);
    matrix3_t m = {0};
 ...
 * @param sz Z 轴缩放因子
 */
static inline matrix3_t Lib_Math_Mat3Scale(float sx, float sy, float sz)
{
    matrix3_t m = {0};
    m.data[0][0] = sx;
    m.data[1][1] = sy;
 ...
 */
static inline matrix3_t Lib_Math_Mat3ShearXY(float sxy)
{
    matrix3_t m = Lib_Math_Mat3Identity();
    m.data[0][1] = sxy;
    return m;
}
 ...
 */
static inline matrix3_t Lib_Math_Mat3ShearXZ(float sxz)
{
    matrix3_t m = Lib_Math_Mat3Identity();
    m.data[0][2] = sxz;
    return m;
 ...
 */
static inline matrix3_t Lib_Math_Mat3ShearYZ(float syz)
{
    matrix3_t m = Lib_Math_Mat3Identity();
    m.data[1][2] = syz;
    return m;
 ...
 * @brief 4x4 单位矩阵
 */
static inline matrix4_t Lib_Math_Mat4Identity(void)
{
    matrix4_t m = {0};
    m.data[0][0] = 1.0f;
    m.data[1][1] = 1.0f;
 ...
 */
static inline matrix4_t Lib_Math_Mat4Mul(matrix4_t m1, matrix4_t m2)
{
    matrix4_t result = {0};
    int i, j, k;
    for (i = 0; i < 4; i++)
 ...
 * @brief 4x4 矩阵转置
 */
static inline matrix4_t Lib_Math_Mat4Transpose(matrix4_t m)
{
    matrix4_t result;
    int i, j;
    for (i = 0; i < 4; i++)
 ...
 */
static inline vector3_t Lib_Math_Mat4MulPoint(matrix4_t m, vector3_t v)
{
 ...
 */
static inline vector3_t Lib_Math_Mat4MulDir(matrix4_t m, vector3_t v)
{
    vector3_t result;
    result.x = m.data[0][0] * v.x + m.data[0][1] * v.y + m.data[0][2] * v.z;
    result.y = m.data[1][0] * v.x + m.data[1][1] * v.y + m.data[1][2] * v.z;
 ...
 * @param angle 旋转角度 (rad)
 */
static inline matrix4_t Lib_Math_Mat4RotationX(float angle)
{
    float c = Lib_Math_Cos(angle);
    float s = Lib_Math_Sin(angle);
    matrix4_t m = {0};
 ...
 * @param angle 旋转角度 (rad)
 */
static inline matrix4_t Lib_Math_Mat4RotationY(float angle)
{
    float c = Lib_Math_Cos(angle);
    float s = Lib_Math_Sin(angle);
    matrix4_t m = {0};
 ...
 * @param angle 旋转角度 (rad)
 */
static inline matrix4_t Lib_Math_Mat4RotationZ(float angle)
{
    float c = Lib_Math_Cos(angle);
    float s = Lib_Math_Sin(angle);
    matrix4_t m = {0};
 ...
 * @param sz Z 轴缩放因子
 */
static inline matrix4_t Lib_Math_Mat4Scale(float sx, float sy, float sz)
{
    matrix4_t m = {0};
    m.data[0][0] = sx;
    m.data[1][1] = sy;
 ...
 * @param tz Z 方向平移
 */
static inline matrix4_t Lib_Math_Mat4Translate(float tx, float ty, float tz)
{
    matrix4_t m = Lib_Math_Mat4Identity();
    m.data[0][3] = tx;
    m.data[1][3] = ty;
    m.data[2][3] = tz;
    return m;
 ...
 * @note X 坐标根据 Y 坐标剪切
 */
static inline matrix4_t Lib_Math_Mat4ShearXY(float sxy)
{
    matrix4_t m = Lib_Math_Mat4Identity();
    m.data[0][1] = sxy;
    return m;
 ...
 * @note X 坐标根据 Z 坐标剪切
 */
static inline matrix4_t Lib_Math_Mat4ShearXZ(float sxz)
{
    matrix4_t m = Lib_Math_Mat4Identity();
    m.data[0][2] = sxz;
    return m;
 ...
 * @note Y 坐标根据 Z 坐标剪切
 */
static inline matrix4_t Lib_Math_Mat4ShearYZ(float syz)
{
    matrix4_t m = Lib_Math_Mat4Identity();
    m.data[1][2] = syz;
    return m;
 ...
 * @note 不适用于包含缩放或剪切的矩阵
 */
static inline matrix4_t Lib_Math_Mat4InverseAffine(matrix4_t m)
{
    /* 提取旋转子矩阵 (左上 3x3) */
    matrix3_t r;
    ...
    matrix3_t r_trans = Lib_Math_Mat3Transpose(r);

    /* 提取平移向量 */
    vector3_t t = {m.data[0][3], m.data[1][3], m.data[2][3]};
    vector3_t neg_rt_t = Lib_Math_Vec3Scale(Lib_Math_Mat3MulVec3(r_trans, t), -1.0f);
