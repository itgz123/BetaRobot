...
#ifndef __LIB_MATH_ANGLE_H
#define __LIB_MATH_ANGLE_H
 ...
 */
static inline float Lib_Math_AngleDiff(float from, float to)
{
    float diff = to - from;
    if (diff > M_PI)
    {
 ...
 */
static inline float Lib_Math_WrapAngle0To2PI(float angle)
{
 ...
 */
static inline float Lib_Math_WrapAngleNegPIToPI(float angle)
{
    angle = fmodf(angle, M_2PI);
    if (angle >= M_PI)
    {
 ...
 */
static inline float Lib_Math_WrapAngle(float angle, float min_val, float max_val)
{
