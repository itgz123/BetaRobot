...
#ifndef __LIB_MATH_TRANSFORM_H
#define __LIB_MATH_TRANSFORM_H
 ...
 *            y' = x*sin + y*cos
 */
static inline vector2_t Lib_Math_Vec2Rotate(vector2_t v, float angle)
{
 ...
 * @note 零轴返回原向量
 */
static inline vector3_t Lib_Math_Vec3RotateAxis(vector3_t v, vector3_t axis, float angle)
{
    float len = Lib_Math_Vec3Length(axis);
    if (len < 1e-8f)
    {
        return v; /* 零轴，返回原向量 */
    }
    ...
    /* 归一化旋转轴 */
    vector3_t k = Lib_Math_Vec3Scale(axis, 1.0f / len);
    float c = Lib_Math_Cos(angle);
    float s = Lib_Math_Sin(angle);
    float one_minus_c = 1.0f - c;

    /* v_rot = v * cos(θ) + (k × v) * sin(θ) + k * (k · v) * (1 - cos(θ)) */
    vector3_t k_cross_v = Lib_Math_Vec3Cross(k, v);
    vector3_t k_dot_v_k = Lib_Math_Vec3Scale(k, Lib_Math_Vec3Dot(k, v));

    vector3_t term1 = Lib_Math_Vec3Scale(v, c);
    vector3_t term2 = Lib_Math_Vec3Scale(k_cross_v, s);
    vector3_t term3 = Lib_Math_Vec3Scale(k_dot_v_k, one_minus_c);

    vector3_t result = Lib_Math_Vec3Add(term1, term2);
    result = Lib_Math_Vec3Add(result, term3);
    return result;
}
 ...
 * @note 零轴返回单位矩阵
 */
static inline matrix3_t Lib_Math_Mat3FromAxisAngle(vector3_t axis, float angle)
{
    float len = Lib_Math_Vec3Length(axis);
    if (len < 1e-8f)
    {
 ...
 */
static inline matrix4_t Lib_Math_Mat4TRS(vector3_t t, quaternion_t r, vector3_t s)
{
