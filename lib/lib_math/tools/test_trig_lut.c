 ...
 * @note 用法（在 tools/ 目录下）：
 *       QUARTER（默认）：
 *       gcc -O2 -Wall -Wextra -DLIB_MATH_TRIG_TABLE_SIZE=2048 \
 *           -DTEST_MODE_A_TARGET=1.1920928955078125e-07 \
 *           -I .. -o test_trig_lut test_trig_lut.c ../lib_math_trig_lut.c -lm
 *       FULL（2π 完整周期表）：
 *       gcc ... -DLIB_MATH_TRIG_TABLE_KIND=LIB_MATH_TRIG_KIND_FULL \
 *           -DLIB_MATH_TRIG_TABLE_SIZE=8192 -DTEST_MODE_A_TARGET=... ...
 *       ./test_trig_lut
 *
 * @note 两类检验：
 ...
 *         ε = 2^-23 ≈ 1.192e-7）。
 *       - ModeB（整管 float32）：全区域抽样所有点，跑完整 Lib_Math_Sin/Cos/
 *         SinCos/TanLUT（LIB_MATH_TRIG_LUT_USED 时经 lib_math_trig.h 分发，
 ...
 * STANDALONE 由 test_trig_lut.sh 通过 -D 传入，lib_math_trig.h 据此跳过 app_cfg.h；
 * 三个三角函数 Lib_Math_Sin/Cos/SinCos 在 LIB_MATH_TRIG_LUT_USED 下自动走查表。 */
#ifndef LIB_MATH_TRIG_LUT_USED
#define LIB_MATH_TRIG_LUT_USED
#endif
...
#else
        double v0 = (double)Lib_Math_SinTable[i];
...
static void run_pipeline(double theta)
{
    ...
    /* 一致性：cos(x) == sin(x + π/2) */
    {
        double e = fabs((double)Lib_Math_Sin(thf + (float)QUARTER_D) - (double)c);
        if (e > g_cons_max)
        {
            g_cons_max = e;
        }
...
int main(void)
{
    ...
    /* 大角度信息（float32 归一回舍入固有，不作判定） */
    {
        ...
        for (k = 0; k < sizeof(big) / sizeof(big[0]); k++)
        {
            ...
            double e = fabs((double)Lib_Math_SinLUT((float)big[k]) - sin(thf));
            if (e > mx)
            {
                mx = e;
            }
