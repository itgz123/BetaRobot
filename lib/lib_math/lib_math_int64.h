 ...
 *       的等价整数实现：
 *         - Lib_Math_UMulH(a,b) = (a*b)>>64：4 次 32×32→64 umull + 进位累加；
 *         - 除以除数 d 等价于"乘以因子 magic/2^64 再取高 64 位"，
 *           magic = ceil(2^64/d)（Lib_Math_Magic 预计算一次），
 *           商 = Lib_Math_U64Div(v, d, magic)，至多一次修正。
 *       全程 umull + 移位 + 减法，零除法指令、不调用 __aeabi_uldivmod。
 *       供 lib_format（64 位十进制/十六进制）与 bsp_dwt（us 时间戳）共用。
 *
 * 验证（host，/tmp/test_math_int64.c）：Lib_Math_UMulH / Lib_Math_U64Div /
 *   Lib_Math_U64DivMod1000 与 __uint128、/ % 参照对比，小值域全遍历 +
 *   各比特位边界窗口 + 大量随机，全部一致。
 */

#ifndef __LIB_MATH_INT64_H
#define __LIB_MATH_INT64_H
...
/**
 * @brief 64 位高半乘法：Lib_Math_UMulH(a,b) = (a*b)>>64
 ...
 */
static inline uint64_t Lib_Math_UMulH(uint64_t a, uint64_t b)
{
 ...
 */
static inline uint64_t Lib_Math_Magic(uint32_t d)
{
    if (d <= 1u)
    {
        return 0u;
 ...
 * @param magic 除数 d 的魔数（Lib_Math_Magic(d) 预计算）
 * @note 定点小数字因子思路：因子 = magic/2^64，商 = floor(v×因子) =
 *       umulh(v, magic)。对 d 不整除 2^64，q0 ∈ {⌊v/d⌋, ⌊v/d⌋+1}
 *       （高估 ≤ 1：q0 = ⌊v/d + (r/d)·(d-rem)/d⌋ < q + 2，见 Lib_Math_UMulH
 *       注释思路），一次修正：q0·d 溢出（umulh(q0,d)≠0）或大于 v 则回退 1。
 *       全程 umull + 移位 + 减法，无除法指令、无 __aeabi_uldivmod。
 */
static inline uint64_t Lib_Math_U64Div(uint64_t v, uint32_t d, uint64_t magic)
{
    uint64_t q = Lib_Math_UMulH(v, magic);
    uint64_t prod = q * (uint64_t)d;
    if (Lib_Math_UMulH(q, (uint64_t)d) != 0u || prod > v)
    {
        q--;
    }
    return q;
 ...
 * @note v/1000 = (v/8)/125：
 *       - v8 = v>>3（< 2^61）；商 q0 = Lib_Math_UMulH(v8, floor(2^64/125))，
 ...
 *       全程 umull + 移位 + 减法，无除法指令、无 __aeabi_uldivmod。
 */
static inline uint64_t Lib_Math_U64DivMod1000(uint64_t v, uint64_t *rem)
{
    uint64_t v8 = v >> 3;
    uint64_t q0 = Lib_Math_UMulH(v8, 0x20c49ba5e353f7cull);
