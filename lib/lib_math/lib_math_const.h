...
#ifndef __LIB_MATH_CONST_H
#define __LIB_MATH_CONST_H
 ...
 * @return 限制后的值
 */
static inline float Lib_Math_Clamp(float val, float min, float max)
{
    if (val > max)
        return max;
    if (val < min)
 ...
 * @return |x|
 */
static inline float Lib_Math_Fabs(float x)
{
    return (x >= 0.0f) ? x : -x;
}

#endif /* __LIB_MATH_CONST_H */
