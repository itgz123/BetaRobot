...
用法：
    python lib_crc_tables_gen.py          # 输出到本文件所在目录
"""
...
def build():
    # 输出到 lib_crc/（脚本位于 lib_crc/tools/ 下，取上级目录）
    ...
    lines_h = []
    lines_h.append("/**")
    lines_h.append(" * @file lib_crc_tables.h")
    lines_h.append(" * @brief 常用 CRC 算法 Flash 表 + 算法/查表描述符（由 lib_crc_tables_gen.py 生成）")
    lines_h.append(" *")
    lines_h.append(" * 表为 const，放 FLASH：零 RAM、零运行时建表。用法：")
    lines_h.append(" *   uint32_t c = LIB_CRC_TableCalc(&LIB_CRC_TBL_CRC8, data, len);")
    lines_h.append(" * 重新生成：python lib_crc_tables_gen.py")
    lines_h.append(" */")
    lines_h.append("")
    lines_h.append("#ifndef __LIB_CRC_TABLES_H")
    lines_h.append("#define __LIB_CRC_TABLES_H")
    lines_h.append("")
    lines_h.append('#include "lib_crc.h"')
    lines_h.append("")
    lines_h.append("/* %d 张查表（256 项 uint32_t，const FLASH） */" % len(tables))
    for tname in tables:
        lines_h.append("extern const uint32_t LIB_CRC_TABLE_%s[256];" % tname)
    lines_h.append("")
    lines_h.append("/* %d 个算法描述符 */" % len(ALGOS))
    for name, *_ in ALGOS:
        lines_h.append("extern const LIB_CRC_Algo_t LIB_CRC_ALGO_%s;" % name)
    lines_h.append("")
    lines_h.append("/* %d 个查表描述符（算法 + 表） */" % len(ALGOS))
    for name, *_ in ALGOS:
        lines_h.append("extern const LIB_CRC_Table_t LIB_CRC_TBL_%s;" % name)
    lines_h.append("")
    lines_h.append("#endif /* __LIB_CRC_TABLES_H */")
    lines_h.append("")

    # ---- 生成 .c ----
    lines_c = []
    lines_c.append("/**")
    lines_c.append(" * @file lib_crc_tables.c")
    lines_c.append(" * @brief 常用 CRC 算法 Flash 表（由 lib_crc_tables_gen.py 生成，勿手改）")
    lines_c.append(" */")
    lines_c.append("")
    lines_c.append('#include "lib_crc.h"')
    ...
    for tname, vals in tables.items():
        ...
        lines_c.append("const uint32_t LIB_CRC_TABLE_%s[256] = {" % tname)
    ...
    for name, tname, init, width, poly, xorout, refin, refout, check in ALGOS:
        lines_c.append("const LIB_CRC_Algo_t LIB_CRC_ALGO_%s = {" % name)
        ...
        lines_c.append("};")
        lines_c.append("")
        lines_c.append("const LIB_CRC_Table_t LIB_CRC_TBL_%s = {" % name)
        lines_c.append("    .algo  = &LIB_CRC_ALGO_%s," % name)
        lines_c.append("    .table = LIB_CRC_TABLE_%s," % tname)
        lines_c.append("};")
        lines_c.append("")
    ...
    c_path = os.path.join(out_dir, "lib_crc_tables.c")
