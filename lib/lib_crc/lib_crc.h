/**
 * @file lib_crc.h
 * @brief lib层CRC计算封装（直接计算 / Flash 表 / 自定义表 / 通用查表）
 *
 * 设计（分层降级，任一层都可用）：
 *   1. LIB_CRC_Direct    逐位直接计算，可算任意算法（Poly/Init/XorOut/RefIn/RefOut
 *                        五个自由度全覆盖），支持 CRC-7/MMC、CRC-8、CRC-16、CRC-32
 *                        等全部常用标准算法 —— 兜底层，无需任何表。
 *   2. LIB_CRC_TableCalc + Flash 内置表（lib_crc_tables.h）：常用算法预生成 const 表
 *                        （零 RAM、零运行时建表），查表计算每字节 1 次查表替代 8 次位循环。
 *   3. LIB_CRC_CUSTOM_DEF 宏定义自定义算法表（定义时只定大小 256 项），运行时调用
 *                        LIB_CRC_GenTable（先校验表大小与算法是否匹配，匹配才生成），
 *                        再用 LIB_CRC_TableCalc 计算。
 *
 * 表统一为 256 项 uint32_t（1 字节输入 → 查表项），Flash const 表与 RAM 生成表格式一致，
 * LIB_CRC_TableCalc 两者通用。
 */
...
typedef struct
{
    const LIB_CRC_Algo_t *algo; /* 算法描述 */
 ...
 * @param algo CRC算法描述（见 LIB_CRC_Algo_t）
...
/* 自定义算法表定义宏（对齐 COMM_DEF 风格）：定义查表描述符 LIB_CRC_Table_t name（表名）
 * + 内部 256 项 RAM 表 name##_table（只定大小）。算法参数（Poly/Init/XorOut/RefIn/RefOut）
 * 不写在宏里，由用户定义 LIB_CRC_Algo_t 常量并绑定到 name.algo，调用 LIB_CRC_GenTable
 * 生成表时传入（生成时设置算法）。
 *
 *   LIB_CRC_CUSTOM_DEF(name) →
 ...
 *
 * @example 自定义算法流程：
 *   LIB_CRC_CUSTOM_DEF(my_crc);                 // 1. 定义描述符 my_crc + 表（只定大小）
 *   static const LIB_CRC_Algo_t my_crc_algo =   // 2. 算法参数
